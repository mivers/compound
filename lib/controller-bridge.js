...
ControllerBrigde.prototype.callControllerAction = function (controller, action, req, res, next) {
    ...
    ctl.perform(action, req, res, function(err) {
        ...
        if (ctl && ctl._backToPool) {
            if (ctl.occupied) {
                throw new Error("Sending a occupied controller back to the pool");
            }
            ctl._backToPool();
        } else {
            ctl = null;
